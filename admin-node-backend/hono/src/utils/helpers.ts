...
export async function getEntityById<T extends Document>(
  ...
  try {
    ...
    if (!result) {
      c.status(404);
      await c.json({ error: `Entity with id ${id} not found` });
      return null;
    }
    return result as T;
  } catch (error) {
    ...
    c.status(500);
    await c.json({ error: 'Failed to retrieve entity', details: (error as Error).message });
    return null;
  }
}
...
export function validateRequiredFields(
  ...
  for (const [fieldName, fieldValue] of Object.entries(fields)) {
    if (!fieldValue || (typeof fieldValue === 'string' && fieldValue.trim() === '')) {
      c.status(400);
      c.json({ error: `Missing required field: ${fieldName}` });
      return false;
    }
  }
  return true;
